...
class ANTLRVisitor extends AbstractParseTreeVisitor<IStrategoTerm> {
	...
	@Override
	public IStrategoTerm visit(ParseTree tree) {
		return super.visit(tree);
	}

	@Override
	public IStrategoTerm visitChildren(RuleNode node) {
	...
	@Override
	public IStrategoTerm visitErrorNode(ErrorNode node) {
	...
	@Override
	public IStrategoTerm visitTerminal(TerminalNode node) {
	...
	protected IStrategoConstructor makeConstructor(RuleNode node) {
		IStrategoConstructor constructor = termFactory.makeConstructor(
				StringUtils.capitalize(Trees.getNodeText(node, ruleNames)),
				node.getChildCount()
		);
