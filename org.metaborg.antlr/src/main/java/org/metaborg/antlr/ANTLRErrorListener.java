...
class ANTLRErrorListener extends BaseErrorListener {
    ...
    @Override public void syntaxError(Recognizer<?, ?> recognizer, Object offendingSymbol, int line,
        ...
        final int endOffset = charPositionInLine + length(offendingSymbol);
        final ISourceRegion region =
            new SourceRegion(charPositionInLine, line, charPositionInLine, endOffset, line, endOffset);
        final IMessage message = new Message(msg, MessageSeverity.ERROR, MessageType.PARSER, source, region, e);
    ...
    /**
     * Computes the length of the offending symbol. If the offending symbol is not a token or of it spans multiple lines
     * 0 is returned.
     * 
     * @return
     */
